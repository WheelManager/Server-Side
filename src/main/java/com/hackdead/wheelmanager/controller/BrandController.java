...
@Api(tags="Brand", value = "Service Web RESTful de brands")
public class BrandController {
    ...
    public ResponseEntity<List<Brand>> findByName(@PathVariable("name") String name){
        try{
            List<Brand> brands = brandService.findByName(name);
            if(brands.size()>0)
                return new ResponseEntity<List<Brand>>(brands, HttpStatus.OK);
            else
                return new ResponseEntity<List<Brand>>(HttpStatus.NOT_FOUND);
